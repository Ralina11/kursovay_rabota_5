...
def create_table_employers_postgres() -> None:
    ...
    cur.execute('''CREATE TABLE employers
            (employers_id int PRIMARY KEY NOT NULL,
...
def push_data_in_employers(file_json: str = 'employers_data.json'):
    ...
    for row in data["items"]:
        items = tuple([row["id"], row["name"], row["url"], row["open_vacancies"]])
    ...
    for row in result_companies:
        cur.execute("INSERT INTO employers VALUES (%s, %s, %s, %s)", row)
    con.commit()
    con.close()
    ...
    for key, value in data.items():
            ...
            for r in row:
                ...
                if r["salary"] is not None:
                    ...
                    salary_from = 0
                    salary_to = 0
                items = tuple([id, employers_id, vacancies_name, url, salary_from, salary_to])
